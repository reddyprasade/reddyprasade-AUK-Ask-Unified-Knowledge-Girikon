const {getUUID4} = require("../utils");

module.exports = (db) => {
    const Widgets = db.sequelize.define('widgets', {
            wg_id: {type: db.Sequelize.DataTypes.UUID, primaryKey: true, defaultValue: db.Sequelize.UUIDV4, allowNull: false},

            wg_server_url: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: 'https://api.girikon.ai/api/v1/public/chat_gpt'},
            wg_user_id: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: ''},
            wg_initial_payload: {type: db.Sequelize.DataTypes.STRING(100), allowNull: false, defaultValue: '/greet'},
            wg_metadata: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '{}'},
            wg_bot_avatar: {
                type: db.Sequelize.DataTypes.TEXT,
                allowNull: false,
                defaultValue: 'data:image/png;base64,'
            },
            wg_widget_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#a78bfa'},
            wg_text_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4c1d95'},
            wg_user_msg_background_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#e1d7ff'},

            wg_bot_title: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: 'Girikon Chatbot'},
            wg_bot_sub_title: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: 'Knowledge Base Assistant'},
            wg_bot_msg_background_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#f3f4f6'},
            wg_bot_response_delay: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: ''},

            wg_chat_header_css__text_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4c1d95'},
            wg_chat_header_css__background_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#a78bfa'},
            wg_chat_header_css__enable_bot_avatar_border: {type: db.Sequelize.DataTypes.BOOLEAN, allowNull: false, defaultValue: true},

            wg_chat_header_text_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4c1d95'},
            wg_bot_msg_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4b5563'},
            wg_user_msg_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4c1d95'},
            wg_embedded: {type: db.Sequelize.DataTypes.BOOLEAN, allowNull: false, defaultValue: false},

            wg_buttons_css__color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4c1d95'},
            wg_buttons_css__background_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#e1d7ff'},
            wg_buttons_css__border_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4b5563'},
            wg_buttons_css__border_width: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '0px'},
            wg_buttons_css__border_radius: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '999px'},
            wg_buttons_css__hover_background_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#FFFFFF'},
            wg_buttons_css__hover_color: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '#4b5563'},
            wg_buttons_css__hover_border_width: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: '1px'},
            wg_buttons_css__enable_hover: {type: db.Sequelize.DataTypes.BOOLEAN, allowNull: false, defaultValue: false},

            wg_whitelist_domains: {type: db.Sequelize.DataTypes.TEXT, allowNull: false, defaultValue: "http://localhost:3000"},
            wg_collection_id_fk: {type: db.Sequelize.DataTypes.UUID, allowNull: true, defaultValue: null},
            wg_api_key_id_fk: {type: db.Sequelize.DataTypes.UUID, allowNull: true, defaultValue: null},

            wg_status: {type: db.Sequelize.DataTypes.STRING(20), allowNull: false, defaultValue: 'active'},
            wg_created_by: {type: db.Sequelize.DataTypes.UUID},
            wg_created_at: {type: db.Sequelize.DataTypes.DATE, allowNull: false, defaultValue: db.Sequelize.NOW},
            wg_updated_by: {type: db.Sequelize.DataTypes.UUID},
            wg_updated_at: {type: db.Sequelize.DataTypes.DATE, allowNull: false, defaultValue: db.Sequelize.NOW}
        },
        {
            indexes: [
                {unique: true, fields: ['wg_id']},
            ],
            timestamps: true,
            underscrored: true,
            createdAt: false,
            updatedAt: false,
            hooks: {
                beforeCreate: async (o, options) => {
                    o.wg_id = getUUID4();
                    o.wg_created_at = new Date();
                    o.wg_updated_at = new Date();
                },
                beforeUpdate: async (o, options) => {
                    o.wg_updated_at = new Date();
                }
            }
        });

    Widgets.add = async (req) => {
        let data = req && req.body ? req.body : req;
        let user = req && req.user ? req.user : req;

        data.wg_user_id = user.id;
        data.wg_created_by = user.id;
        data.wg_updated_by = user.id;
        data.wg_server_url = `${process.env.API_URL}/api/v1/widgets/ask`;

        return Widgets.create(data);
    }

    Widgets.edit = async (data) => {
        return Widgets.update(data, {where: {wg_id: data.wg_id}});
    }

    Widgets.delete = async (data) => {
        return Widgets.destroy({where: {wg_id: data.wg_id}});
    }

    Widgets.getOne = async (wg_id) => {
        console.log({where: {wg_status: 'active', wg_id: wg_id},nest: true, plain: true});
        return await Widgets.findOne({where: {wg_status: 'active', wg_id: wg_id},nest: true, plain: true});
    }

    Widgets.getAll = async (data) => {
        return Widgets.findAll({
            where: {
                wg_status: 'active',
                wg_created_by: data.wg_created_by
            },
        });
    }

    Widgets.getAllInActive = async (data) => {
        return Widgets.findAll({
            where: {
                wg_status: 'inactive',
                wg_created_by: data.wg_created_by
            },
        });
    }

    return Widgets;
};